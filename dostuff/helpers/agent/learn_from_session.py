from dostuff.helpers.agent.constants import  SYSTEM_INSTRUCTION_FOR_SELF_LEARNING
...
async def learn_from_session(steps_history: list[dict], mcp_client: MCPClient, session_id: str):
    ...
    if not conversation_text.strip() or len(conversation_text) < 100:
        ...
        emit("[Learning Loop] Transcript too short for skill reflection.", msg_type="system")
        return
